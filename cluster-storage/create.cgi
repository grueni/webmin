...
print &ui_table_row($text{'index_cmd'},&ui_textbox("cmd", undef, 60));
...
@opts = ( [ "ALL", $text{'index_all'} ],[ "*", $text{'index_this'} ] );
foreach $s (grep { $_->{'user'} } sort { $a->{'host'} cmp $b->{'host'} } &servers::list_servers()) {
	push(@opts, [ $s->{'host'},$s->{'host'}.($s->{'desc'} ? " (".$s->{'desc'}.")" : "") ]);
	}
...
print &ui_table_row(
	$text{'index_server'},
	&ui_select("server", [ split(/ /, $config{'server'}) ],\@opts, 10, 1));
