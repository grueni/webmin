...
our %sesdatas;
# my $data = &sesmanager::discover_getinventory();
...
#		print "<pre>".Dumper(\@$rv)."</pre>";		
		$sesdatas{$s->{'host'}} = $rv->[1];
		}
	$p++;
	}

&Storable::store(\%sesdatas,$tempfileInventories);

print "<pre>".$tempfileInventories."</pre>";
# print "<pre>".Dumper(\%sesdatas)."</pre>";
# my $test = &retrieve($tempfile);
