...
if ($status{$in{'dev'}}{type} eq "disk") { 
	my $d = $status{$in{'dev'}}{name};
	$d =~ s/\/dev\/rdsk\///ig;
	print &ui_columns_row([ "<a href='/sesmanager/status.cgi?disk=$d'>$status{$in{'dev'}}{name}</a>", 
			$status{$in{'dev'}}{state}, $status{$in{'dev'}}{read}, $status{$in{'dev'}}{write}, $status{$in{'dev'}}{cksum}]);
} else {
...
if ($status{$in{'dev'}}{parent} =~ 'pool') 
{
	...
	print ui_columns_start([ "Parent", "Type", "State", "Read", "Write", "Cksum" ]);
	print ui_columns_row(["<a href='config-vdev.cgi?pool=$in{'pool'}&dev=$status{$in{'dev'}}{parent}'>$status{$parent}{name}</a>", 
		$status{$parent}{type},$status{$parent}{state}, $status{$parent}{read}, $status{$parent}{write}, $status{$parent}{cksum}]);
	print ui_columns_end();
